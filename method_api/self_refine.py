...
from utils.openai_parameter import args, openai_api_key
import time
...
# read predict data for refine
with open(args.input_path, 'r', encoding='utf-8') as fin:
    inputs_text =[]
    for line in fin.readlines():
        inputs_text.append(json.loads(line))
...
Find out if there are any missing slots, slot category errors, intent errors, or any other issues.
If there is no error, you do not need to revise.
If there is an error, find it out.
"""

iterative_prompt = """
Regenerate the annotations in the form of "Intent=INTENT_NAME;Slot1=VALUE1;Slot2=VALUE2;..."
"""

# testing
with open(args.output_path, 'w', encoding='utf-8') as fout:
    for idx, sentence in enumerate(tqdm(inputs_text[390:])):
        try:
            # refine
            history = [
                HumanMessage(content=prompt+sentence['text']),
                AIMessage(content=sentence['pred_slot']),
                HumanMessage(content=refine_prompt)
            ]
            out = chat(history)
            sentence['pred_error'] = out.content
            history = [
                HumanMessage(content=prompt+sentence['text']),
                AIMessage(content=sentence['pred_slot']),
                HumanMessage(content=refine_prompt),
                AIMessage(content=sentence['pred_error']),
                HumanMessage(content=iterative_prompt)
            ]
            out = chat(history)
            sentence['pre_pred_slot'] = sentence['pred_slot']
            sentence['pred_slot'] = out.content
